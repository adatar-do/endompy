"""ENFT dictionary editions, independent from poverty methodologies."""
...
def get_dict(version=None, at=None, con=None):
    """Load an exact ENFT revision, or a documented edition applicable at a date.
    ...
    """
    if con is not None:
        return RevisionRegistry(con).load("enft", version=version, at=at)
...
def dict_versions(con=None):
    ...
    if con is not None:
        return RevisionRegistry(con).list_versions("enft")
...
def register_dict(con, dictionary, version, valid_from=None, valid_to=None, **kwargs):
    ...
    """
    return RevisionRegistry(con).register(dictionary, version, dictionary_id="enft",
