...
canonical = "https://endomer-py.github.io/encftr/" if args.kind == "r" else "https://adatar-do.github.io/endompy/"
...
if args.kind == "r":
    ...
    for index in (root / "search.json", root / "en/search.json"):
        ...
        replacement = marker + '''
    // ENCFT: resolve results relative to this edition's search index.
    data = data.map(function(item) {
      item.path = new URL(item.path, response.url).href;
      return item;
    });'''
        if "// ENCFT: resolve results" not in javascript:
        ...
        if "// ENCFT: encode the query" not in javascript:
            if javascript.count(old_target) != 1:
                raise SystemExit("Review changed pkgdown result navigation")
            javascript = javascript.replace(old_target, '''    // ENCFT: encode the query and omit absent section anchors.
    ...
    window.location.href = destination.href;''')
        ...
        if "// ENCFT: share the initial index request" not in javascript:
            ...
            javascript = javascript[:start] + '''  // ENCFT: share the initial index request with the first typed query.
  var fuse;
  var fuseLoading;
...
for page in pages:
    ...
    for node in soup.select("[data-encft-language], link[hreflang]"): node.decompose()
    ...
    link = soup.new_tag("a", href=href, attrs={"data-encft-language": "true", "hreflang": "es" if language == "en" else "en"})
    ...
    if args.kind == "r":
        ...
        if not navs:
            ...
            item = soup.new_tag("li", attrs={"class": "nav-item", "data-encft-language": "true"})
...
for page in pages:
    ...
    for tag in soup.select("[href], [src]"):
        ...
        if path.startswith("/"):
            prefix = "/encftr/" if args.kind == "r" else "/endompy/"
